...
const oddEven = [1, 2];
const pickOddEven = [
  document.getElementById("player-picks-odd"),
  document.getElementById("player-picks-even"),
];
...
let binaryBase = true;
const switchBase = [
  document.getElementById("switch-to-binary"),
  document.getElementById("switch-to-decimal"),
];
let userBet;
let userPick;
let computerPick;
...
const handleSwitchBaseEvent = (e) => {
  if (e.target.id == "switch-to-binary") {
    ...
    if (!binaryBase) {
      ...
      if (!isNaN(parseInt(resultDisplay.innerText)))
        resultDisplay.innerText = parseInt(resultDisplay.innerText).toString(2);
      // see if the value of the computer's choice needs to be displayed binary:
      if (!isNaN(parseInt(computerChoiceDisplay.innerText)))
        computerChoiceDisplay.innerText = parseInt(
    ...
    if (binaryBase) {
      ...
      if (!isNaN(parseInt(resultDisplay.innerText)))
        resultDisplay.innerText = parseInt(resultDisplay.innerText, 2).toString(
          10
        );
      // see if the value of the computer's choice needs to be displayed binary:
      if (!isNaN(parseInt(computerChoiceDisplay.innerText)))
...
const generateResult = () => {
  ...
  console.log(
    `Resulting value= ${result} and the result is ${
      oddEvenText[result % oddEvenText.length]
    }`
  );
  ...
  switch (userBet + (result % oddEvenText.length)) {
    case "odd0":
      talkBackDisplay.innerText =
        "Your bet was odd. Result is even. You lost this round.";
      break;
    case "odd1":
      talkBackDisplay.innerText =
        "Your bet was odd. Result is odd. You win this round.";
      break;
    case "even0":
      talkBackDisplay.innerText =
    ...
    case "even1":
      talkBackDisplay.innerText =
        "Your bet was even. Result is odd. You lost this round.";
      break;
  }
